...
export default function MoveToComponent({
    ...
    const {bookId, bookTags} = notecardCredentials;
    ...
    const handleNotecardTagsUpdate = (tag: string): void => {
        ...
        const isTagInBookTags = currentNotecardTags?.bookTags.includes(capitalizedTagName);
                        ...
                        <ul>
                            {cntx.map((tag, index) => (
                                <li
                                    ...
                                    className={currentNotecardTags?.bookTags.includes(tag.at(0)?.toUpperCase() + tag.slice(1)) ? "inFolder" : undefined}
                                >
