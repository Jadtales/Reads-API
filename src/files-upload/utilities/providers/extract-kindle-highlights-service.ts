...
export class ExtractKindleHighlightsServiceUtil {
    ...
    public extractKindleHighlights(kindleFileContent: string): {
        ...
        const normalizedContent = kindleFileContent.replace(/\s*={10,}\s*/g, notesSeparator); // Normalize separators
        ...
        for (const highlight of noteCardsArray) {
            ...
            if (!highlightText) {
                exceptions.push(`Skipped due to empty highlight text: ${highlightText}`);
