...
export class NotificationsService {
    ...
    async getUserNotifications(userId: string): Promise<Notification[]> {
        ...
        const doesUserHasAnyNotifications = await this.notificationsRepository.find({
            where: {notificationRecipientId: Equal(doesUserExist.id)}
        })

        if (doesUserHasAnyNotifications) {
            // query all notifications of The Recipient
            return await this.notificationsRepository
                .createQueryBuilder('notification')
                .where('notifications.notificationRecipientId = :userId', {userId: doesUserExist.id})
    ...
    async createNotification(notificationMessage: Notifications_dto): Promise<Notification> {
        // save received notifications in the database
        let notification;
        try {
            notification = this.notificationsRepository.create({
                ...notificationMessage,
                notificationRecipientId: {id: notificationMessage.notificationRecipientId},
    ...
    async createNotificationForMany(senderId: string, notificationMessage: NotificationData_dto) {
        // fetch user followers
        const userFollowers = await this.followFollowingRelationServices.getFollowers(senderId);
